...
def get_item_category(content_core_item) -> str:
    """Retorna a categoria adequada do item a partir de suas classes.

    Arguments:
        content_core_item {request_html.Element} -- Objeto Element do elemento div alvo da função.
...
def get_item_details(
    ...
    Arguments:
        content_core_item {request_html.Element} -- Objeto Element do elemento div do objeto a ser detalhado.
...
def get_page_content_items(
    ...
    Arguments:
        page_html {request_html.Element} -- Objeto Element da página.
...
def get_content_tree(inital_url: str, session) -> List[Dict[str, str]]:
    """Gera uma árvore de diretórios a partir da URL inicial definida.

    Arguments:
        inital_url {str} -- URL do diretório no topo da árvore.
        session {request_html.HTMLSession} -- Objeto HTMLSession para execução das requisições HTTP.
